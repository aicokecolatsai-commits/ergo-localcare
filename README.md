...
一套專為**演講、工作坊、企業培訓與校園講座**打造的即時人因工程互動 Web 應用。結合**高階工作者精力管理**與**北歐肌肉骨骼問卷 (NMQ) 實證人因工程架構**。
...
1. **學員端 (`index.html`)**：
   - 支援 **4 大族群切換**（💼 辦公室久坐族、🎓 學生研讀族、🏪 久站服務族、🏭 技術操作族）。
   - **8 題 NMQ-Lite 動態題庫**（60 秒快速完成），滿分 100 分。
   - 即時產出**個人人因檢核報告卡**與**人因改善行動指引**。
2. **講師大螢幕投影看板 (`dashboard.html`)**：
   - 16:9 專業高對比深色風格。
   - 左側常駐專屬動態 QR Code。
   - 即時跳動連線人數、全場平均健康度、**全場三大痛點排行榜 (TOP 3)** 與**工作站盲點分佈**。
3. **場次管理後台 (`admin.html`)**：
...
├── css/
│   └── custom.css    # 專業防疲勞深色主題與排版
├── js/
│   ├── config.js     # 系統設定與即時資料庫橋接 (支援 Firebase 與本地廣播)
│   ├── questions.js  # 4 大族群題庫、計分權重與改善行動指引
│   ├── app.js        # 學員端答題與視覺化邏輯
